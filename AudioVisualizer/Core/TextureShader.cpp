...
bool TextureShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	error = wcscpy_s(vsFilename, 128, L"../Core/Texture.vs");
	if (error != 0)
	{
		return false;
	}
	...
	error = wcscpy_s(psFilename, 128, L"../Core/Texture.ps");
	if (error != 0)
	{
		return false;
	}
...
bool TextureShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	// This setup needs to match the VertexType struct in the Model class and in the shader
...
void TextureShader::OutputShaderErrorMessge(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	char* compileErrors;
