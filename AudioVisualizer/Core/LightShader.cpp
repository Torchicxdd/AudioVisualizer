...
bool LightShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	error = wcscpy_s(vsFilename, L"../Core/Light.vs");
	if (error != 0)
	{
		return false;
	}
	...
	error = wcscpy_s(psFilename, L"../Core/Light.ps");
	if (error != 0)
	{
		return false;
	}
...
bool LightShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
   ...
   // This setup needs to match the VertexType stucture in the ModelClass and in the shader.
	...
	// Create a teture sampler state description
	...
	ID3D11ShaderResourceView* texture, XMFLOAT3 lightDirection, XMFLOAT4 diffuseColor)
{
	...
	// Set the position of the light constant buffer in the ps
	bufferNumber = 0;

	// Set the light constant buffer in the ps with updated values
